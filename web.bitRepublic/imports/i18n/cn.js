/*----------------------------------------*\
  bitRepublic - cn.js
  @author Evrard Vincent (vincent@ogre.be)
  @Date:   2018-10-20 12:03:43
  @Last Modified time: 2018-11-03 19:46:02
\*----------------------------------------*/
import i18n from 'meteor/universe:i18n';
i18n.addTranslation('zh-CN', "textOverCounter", "A", "这是数据油田快闪流量税和黑客运动期间，所有用户及其机器人生成的数据油田(bitsoil)总量");
i18n.addTranslation('zh-CN', "textOverCounter", "B", "加入我们，让数据经济造福所有人");

i18n.addTranslation('zh-CN', "App", "FixePunchline", "A", "仅网络浏览这一行为就可损失超过10亿欧元的税金");
i18n.addTranslation('zh-CN', "App", "FixePunchline", "B", "数据油田(bitsoil)是数字经济的新能源");
i18n.addTranslation('zh-CN', "App", "FixePunchline", "C", "是时候申请属于您的份额");

i18n.addTranslation('zh-CN', "SliderMenu", "banner__title", "让数据经济造福全人类");

i18n.addTranslation('zh-CN', "Menu", "about", "关于");
i18n.addTranslation('zh-CN', "Menu", "taxbot", "定制税收机器人");
i18n.addTranslation('zh-CN', "Menu", "redistribution", "参与资源再分配");
i18n.addTranslation('zh-CN', "Menu", "installation", "安装");
i18n.addTranslation('zh-CN', "Menu", "whoweare", "关于我们");
i18n.addTranslation('zh-CN', "Menu", "dyourtaxcollectorbot", "定制你的专属税收机器人");
i18n.addTranslation('zh-CN', "Menu", "claim", "申请数据油田税");
i18n.addTranslation('zh-CN', "Menu", "more", "了解更多");
i18n.addTranslation('zh-CN', "Menu", "createTaxbot", "创建你的税收机器人");
i18n.addTranslation('zh-CN', "Menu", "signup", "注册");
i18n.addTranslation('zh-CN', "Menu", "genereate", "创建您的钱包");
i18n.addTranslation('zh-CN', "Menu", "join", "加入运动");
i18n.addTranslation('zh-CN', "Menu", "bot", "机器人");
i18n.addTranslation('zh-CN', "Menu", "actions", "行动");
i18n.addTranslation('zh-CN', "Menu", "createdAt", "创建");
i18n.addTranslation('zh-CN', "Menu", "nextActionAt", "继续创建");
i18n.addTranslation('zh-CN', "Menu", "active", "激活");
i18n.addTranslation('zh-CN', "Menu", "tweet", "推文");
i18n.addTranslation('zh-CN', "Menu", "bitsoils", "数据油田");
i18n.addTranslation('zh-CN', "Menu", "yourBotsContribution", "你的机器人贡献");
i18n.addTranslation('zh-CN', "Menu", "botInfo", "机器人信息");
i18n.addTranslation('zh-CN', "Menu", "chooseaTweet", "选择推文");
i18n.addTranslation('zh-CN', "Menu", "joinCampaign", "加入数据油田运动");
i18n.addTranslation('zh-CN', "Menu", "FINDOUTMORE", "了解更多");
i18n.addTranslation('zh-CN', "Menu", "DESIGNYOURTAXBOT", "定制你的税收机器人");
i18n.addTranslation('zh-CN', "Menu", "designYourTaxbot", "定制你的税收机器人");
i18n.addTranslation('zh-CN', "Menu", "findOutMore", "了解更多");
i18n.addTranslation('zh-CN', "Menu", "claimBitsoiltax", "申请数据油田税");
i18n.addTranslation('zh-CN', "Menu", "whatJobTaxBotDo", "税收机器人职责");
i18n.addTranslation('zh-CN', "Menu", "activateMe", "激活我");
i18n.addTranslation('zh-CN', "Menu", "createNow", "现在创建");
i18n.addTranslation('zh-CN', "Menu", "theRedistributionMechanism", "再分配机制");
i18n.addTranslation('zh-CN', "Menu", "IDMEMBERS", "ID成员");
i18n.addTranslation('zh-CN', "Menu", "WALLETS", "钱包");
i18n.addTranslation('zh-CN', "Menu", "ID", "身份");
i18n.addTranslation('zh-CN', "Menu", "contact", "连接");
i18n.addTranslation('zh-CN', "Menu", "spreadTheWord", "传播信息分享");
i18n.addTranslation('zh-CN', "Menu", "partners", "合作者");
i18n.addTranslation('zh-CN', "Menu", "license", "执照");
i18n.addTranslation('zh-CN', "Menu", "photoCredits", "图片版权");
i18n.addTranslation('zh-CN', "Menu", "faq", "常见问题咨询");
i18n.addTranslation('zh-CN', "Menu", "frequentlyAskedQuestions", "常见问题咨询");
i18n.addTranslation('zh-CN', "Menu", "supportedBy", "赞助商");
i18n.addTranslation('zh-CN', "Menu", "softwareDevelopment", "软件开发者");

i18n.addTranslation('en-US', "Menu", "technicalSupport", "技术支持");
i18n.addTranslation('en-US', "Menu", "translation", "翻译");
i18n.addTranslation('zh-CN', "Menu", "LowTechLab", "低科技艺术实验室");

i18n.addTranslation('zh-CN', "Menu", "yourWallet", "你的钱包");
i18n.addTranslation('zh-CN', "Menu", "idNumber", "身份证号");
i18n.addTranslation('zh-CN', "Menu", "valueOfYourWallet", "钱包价值");
i18n.addTranslation('zh-CN', "Menu", "privateKey", "个人密钥");
i18n.addTranslation('zh-CN', "Menu", "publicKey", "公开密钥");
i18n.addTranslation('zh-CN', "Menu", "botInfo", "机器人信息");
i18n.addTranslation('zh-CN', "Menu", "settings", "设置");
i18n.addTranslation('zh-CN', "Menu", "gotYourTweet.", "已选择您的推文");
i18n.addTranslation('zh-CN', "Menu", "whichWorldLeaderYouWant.", "你想选择哪个国家领导人");
i18n.addTranslation('zh-CN', "Menu", "youDidIt", "成功操作");
i18n.addTranslation('zh-CN', "Menu", "gotYourTweet", "已选择您的推文");
i18n.addTranslation('zh-CN', "Menu", "hey", "你好");
i18n.addTranslation('zh-CN', "Menu", "heyYouDidItOnceAgain", "您已操作过");
i18n.addTranslation('zh-CN', "Menu", "gladToJoinTheBand", "欢迎加入");
i18n.addTranslation('zh-CN', "Menu", "activation", "激活");
i18n.addTranslation('zh-CN', "Menu", "logIn", "登陆");
i18n.addTranslation('zh-CN', "Menu", "youReceiveSoonAnEmailToResetYourPassword", "你将收到一封电子邮件以重置密码");
i18n.addTranslation('zh-CN', "Menu", "stayYpdatedAndGetAccess", "保持更新并访问");
i18n.addTranslation('zh-CN', "Menu", "toYourPersonalBotAccount", "您的专属机器人账户");
i18n.addTranslation('zh-CN', "Menu", "typeYourEmail", "输入您的邮箱");
i18n.addTranslation('zh-CN', "Menu", "typeYourPassword", "输入密码");
i18n.addTranslation('zh-CN', "Menu", "followMe", "跟我来");
i18n.addTranslation('zh-CN', "Menu", "firstName", "名");
i18n.addTranslation('zh-CN', "Menu", "lastName", "姓");
i18n.addTranslation('zh-CN', "Menu", "yourEmail", "你的电子邮件");
i18n.addTranslation('zh-CN', "Menu", "forgotPassword", "忘记密码？");
i18n.addTranslation('zh-CN', "Menu", "spreadTheWord", "传播消息分享");
i18n.addTranslation('zh-CN', "Menu", "alreadyJoinedTheCampaignLogIn", "已经加入本次运动？登陆");

i18n.addTranslation('zh-CN', "App", "about", "A", "title", "活动信息");
i18n.addTranslation('zh-CN', "App", "about", "A", "content", "数据油田(bitsoil)快闪流量税和黑客运动旨在动员使用社交媒体平台或其它科技工具的用户对自己贡献的流量数据征收微税，从而呼吁数字经济财富的公平再分配。这场运动是由一群在Twitter上愉快地刷着流量的机器人领导。他们的目标是让用户认识了解到自己贡献的流量数据的真实价值。");
i18n.addTranslation('zh-CN', "App", "about", "B", "title", "创建您的专属机器人");
i18n.addTranslation('zh-CN', "App", "about", "B", "content", "在数据油田（bitsoil）网站上，参与者可创建他们专属的税收机器人团队，配备一系列预设的税收要求，并将其发送给国家首领或有影响者，例如十大科技巨头的首席执行官，或是用户个人自行选择的国家首领。");
i18n.addTranslation('zh-CN', "App", "about", "C", "title", "数据油田税（Bitsoil Tax)");
i18n.addTranslation('zh-CN', "App", "about", "C", "content", "数据油田税（Bitsoil Tax）提出数字经济中财富再分配的新愿景。近年来，谷歌、苹果和脸谱网等主要科技公司都从提供免费服务以换取用户数据的商业模式中获益，这种模式现已被打破，用户数据的价值已远远超过免费服务的价值。而这些公司也通过数据深入了解自己的用户，产生了对广告商和其他第三方合作更多的吸引力，利润也持续在增加。数据，新的油田，或者我们创造的“数据油田”（bitsoil），到目前为止已经白白浪费放弃权利，而没有考虑这些数据本身的价值以及谁能从中获益。");

i18n.addTranslation('zh-CN', "BotSelector", "banner", "A", "让数字经济造福全人类");
i18n.addTranslation('zh-CN', "BotSelector", "banner", "B", "定制您的专属税收机器人");
i18n.addTranslation('zh-CN', "BotSelector", "presentation", "参与过由机器人领导的运动吗？想成为它的一部分吗？加入我们，从现在开始，通过一系列操作来定制您的专属机器人或机器人团队。如何定制？首先通过选择预设任务和机器人配置来自定义定制社交机器人，接下来选择推特明信片以及机器人目标和运行时间，就可以了！");

i18n.addTranslation('zh-CN', "About", "A", "title", "数据油田(BitSoil)快闪流量税和黑客运动");
i18n.addTranslation('zh-CN', "About", "A", "content", "数据油田(BitSoil)快闪流量税和黑客运动是一项基于互联网的艺术作品。从本质上讲，这项工作是一个数字活动，它可以在线上和线下同时展开。它的内容是在Twitter上一群快乐的社交媒体机器人与一个线下的互动装置之间的一种互动，这是一个恢复数字经济公平与平衡的关键工具，而现有的数字经济则倾向于将财富集中在少数大型科技公司手中。");
i18n.addTranslation('zh-CN', "About", "B", "title", "如何实现？");
i18n.addTranslation('zh-CN', "About", "B", "content", "A", "这项活动要求对用户的数据征税，以塑造一个财富再分配的新愿景。它根源于Tor和IBM的AI-Watson技术，这些技术用于训练一个税收机器人军团。他们根据用户在Twitter上所生成的数据来检测、收集和开采数据油田（bitsoil）。");
i18n.addTranslation('zh-CN', "About", "B", "content", "B", "当发生这种情况时，在用户的帐户中会弹出带有视频的推文，并将其重定向到活动的在线平台上。在这里会有一个机器人亲切地邀请用户参加财富的再分配活动，要求他生成自己的税收机器人，并让其加入到其他税收机器人大军中，共同参与到让科技公司纳税的申诉活动中。这个申诉过程可以无休止地重复进行。");
i18n.addTranslation('zh-CN', "About", "B", "content", "C", "在数据油田税（Bitsoil Tax）网站上，参与者被邀请发起几项行动。从收集数据油田税（bitsoil tax）活动的信息、开采数据油田（bitsoil），到生成他自己的税收机器人，都配备了一系列动作来执行操作。这些行动可以包括定期间隔的将纳税申诉明信片，发送给他所选择的前十大科技巨头公司的首席执行官，或他所选择的国家首领，或者邀请他的朋友们参加活动。");
i18n.addTranslation('zh-CN', "About", "C", "title", "一个由机器人引领的活动");
i18n.addTranslation('zh-CN', "About", "C", "content", "A", "想知道为什么这个活动是由机器人领导的吗？因为它价格低廉但功能强大。它们在政府和企业的掌控之外，却能被社交媒体用户掌握在手中，大部分用户都可以访问这些机器人。机器人不需要像真人一样睡觉，吃饭或照顾孩子。它们由人类精心设计和管理，因此可以很好地完成大规模的任务，并在短时间内覆盖全球受众。");
i18n.addTranslation('zh-CN', "About", "C", "content", "B", "然而这并不是它们的所有功能：机器人在能够为您开采数据油田（bitsoil）的同时，也在为您进行申诉活动。这是此次活动的主要目的之一。所有机器人都配备了个人开采数据油田（bitsoil）计算器。一个小的脚本都会被计算到，并将机器人执行的每个动作转换成为数据油田（bitsoil）。而您所要做的全部事情只是设计出您的机器人。");
i18n.addTranslation('zh-CN', "About", "D", "title", "参与到再分配中来");
i18n.addTranslation('zh-CN', "About", "D", "content", "A", "在活动平台上，人们不仅可以开采数据油田（bitsoil），同时申诉自己的公平份额，他们还可以成为互联网财富再分配新模式中的一份子。所有参与者及其机器人在推特活动期间的所有行为，都是财富机制普遍再分配的原材料。");
i18n.addTranslation('zh-CN', "About", "D", "content", "B", "在活动期间重新分配的数据油田（bitsoil）的总量，以及由他们的机器人所产生的数据油田（bitsoil）的价值，都可以由被参与者从他们的个人账户中得到关注和监控。在网站上，显示屏会实时显示所有参与者钱包里的数据油田（bitsoil）的数量分布情况。");
i18n.addTranslation('zh-CN', "About", "E", "title", "实时监控机器人");
i18n.addTranslation('zh-CN', "About", "E", "content", "洞悉每分钟的信息，并关注监控您的个人帐户上由您的机器人开采的数据油田（bitsoil）。个人客户经理会从创建税收机器人的第一天开始，实时显示它们的发展变化：它们达到的目标，以及开采的数据油田（bitsoil）总量。");


i18n.addTranslation('zh-CN', "Redistribution", "banner__title", "参与资源再分配");
i18n.addTranslation('zh-CN', "Redistribution", "FixePunchline", "A", "创建您的钱包");
i18n.addTranslation('zh-CN', "Redistribution", "FixePunchline", "B", "参与税收系统");

i18n.addTranslation('zh-CN', "Redistribution", "about", "A", "title", "活动性质");
i18n.addTranslation('zh-CN', "Redistribution", "about", "A", "content", "A", "数据油田税（Bitsoil Tax）提出数字经济中财富再分配的新愿景。人们创造出数据这种新能源或称之为\"数据油田\" ，却免费赠送出，至今未曾反思这些数据的价值及其获益者。近年来，谷歌、苹果和脸谱网（Facebook）等主要科技公司都从提供免费服务以换取用户数据的商业模式中获益，这种模式现已被打破，用户数据的价值已远远超过免费服务的价值。而这些公司也通过数据深入了解自己的用户，产生对广告商和其他第三方合作更多的吸引力，利润也持续在增加。");
i18n.addTranslation('zh-CN', "Redistribution", "about", "A", "content", "B", "该提案探讨税收制度可用作在互联网财富方面恢复公平平衡的工具。目标是将繁荣扩展到普遍的国家和国际体系之外。本项目通过建立新的数据油田（由所有人生产的新型人工资源）通用征税机制，来寻求重新平衡财富和权力不平等的途径。");
i18n.addTranslation('zh-CN', "Redistribution", "about", "B", "title", "HOW");
i18n.addTranslation('zh-CN', "Redistribution", "about", "B", "content", "A", "在运动平台上，用户不仅可以开采数据油田(bitsoil)，还可以通过申请个人公平份额，他们也可以成为互联网财富再分配新模式的一部分。");
i18n.addTranslation('zh-CN', "Redistribution", "about", "B", "content", "B", "参与者及其机器人在活动期间推特上的所有行动都是公平的财富再分配的原材料。参与者可通过个人账号追踪活动期间数据油田再分配的总量以及机器人生成的数据油田价值。");
i18n.addTranslation('zh-CN', "Redistribution", "about", "B", "content", "C", "在线平台向所有参与者实时显示数据油田分配情况。获取机器人开采数据油田每分钟的操作信息，创建您的个人钱包，追踪分配给您的数据油田。");

i18n.addTranslation('zh-CN', "Redistribution", "overWalletList", "A", "实时追踪数据油田再分配至参与者钱包。加入并创建您的专属个人钱包。");
i18n.addTranslation('zh-CN', "Redistribution", "overWalletList", "B", "让数据经济造福全人类。");

i18n.addTranslation('zh-CN', "Installation", "banner__title", "ENTER THE BITREPUBLIC");
i18n.addTranslation('zh-CN', "Installation", "banner__description", "Follow live the redistribution of bitsoils");

i18n.addTranslation('zh-CN', "Installation", "info", "A", "hash", "what");
i18n.addTranslation('zh-CN', "Installation", "info", "A", "title", "本次活动 是什么");
i18n.addTranslation('zh-CN', "Installation", "info", "A", "desciption", "数据油田(BitSoil)快闪流量税和黑客运动在正在展出的艺术作品“数据共和国” （bitRepublic）的背景下形成，旨在重新定义和设计一个开放的、人工的、DIY的平台，目的是保证被剥夺权利，被取代或被排斥的人获得无条件的社会经济包容权，在国家无法控制的情况下补偿一些实际的缺陷。");
i18n.addTranslation('zh-CN', "Installation", "info", "B", "hash", "how_it_works");
i18n.addTranslation('zh-CN', "Installation", "info", "B", "title", "本次活动的性质");
i18n.addTranslation('zh-CN', "Installation", "info", "B", "desciption", "A", "数据油田(Bitsoil)快闪流量税和黑客运动直接连接至“数据共和国”（bitRepublic）——目前正在布鲁塞尔GLUON艺术与科技中心展出的一部线下艺术作品。");
i18n.addTranslation('zh-CN', "Installation", "info", "B", "desciption", "B", "在活动期间，推特上的每个操作都会触发数据共和国（bitRepublic）装置内的小型线下票据接收器，将少量的数据油田（bitsoil）随机分配至活动参与者的钱包。每个钱包代表一位数据共和国（bitRepublic）的公民。");
i18n.addTranslation('zh-CN', "Installation", "info", "B", "desciption", "C", "参与者可以在票据打印机上跟踪用户和推特机器人生成的数据油田(Bitsoil)。访问者是财富再分配的一部分，也被要求参与并跟进数据资源再分配的过程。");
i18n.addTranslation('zh-CN', "Installation", "info", "B", "desciption", "D", "本次活动中，该技术作为一个重要工具，在一定程度上恢复基本权利，并在现行的国家和国际体系之外传播繁荣。通过新的税收制度，该项目探索在平等社会中重新分配互联网资源的可能性，无论个人出身、性别以及其他特征。");
i18n.addTranslation('zh-CN', "Installation", "info", "C", "hash", "calendar");
i18n.addTranslation('zh-CN', "Installation", "info", "C", "title", "日历");
i18n.addTranslation('zh-CN', "Installation", "info", "C", "desciption", "A", "该活动艺术作品正在巡回展览，请在下方查询日期信息。");
i18n.addTranslation('zh-CN', "Installation", "info", "C", "desciption", "B", "2018年5月3日至2018年5月5日，于勒文And& Summit Festival展出");
i18n.addTranslation('zh-CN', "Installation", "info", "C", "desciption", "C", "2018年5月17日至2018年5月27日，于布鲁塞尔Gluon Art & Tech Hub展出");
i18n.addTranslation('zh-CN', "Installation", "info", "C", "desciption", "D", "2018年9月6月至2018年9月16日，于林茨Ars Electronica：ERROR The Art of Imperfection展出");
i18n.addTranslation('zh-CN', "Installation", "info", "C", "desciption", "E", "2018年11月16日至2019年2月28日，柏林Drive展厅，ERROR The Art of Imperfection展出");
i18n.addTranslation('zh-CN', "Installation", "info", "D", "hash", "host_the_installation");
i18n.addTranslation('zh-CN', "Installation", "info", "D", "title", "主持安装");
i18n.addTranslation('zh-CN', "Installation", "info", "D", "desciption","在您所在的城市主持安装. <br/>请发送邮件至 <a href=\"{$address}\" >campaign@bitsoil.tax联系我们</a>");

i18n.addTranslation('zh-CN', "Installation", "FixePunchline", "A", "数据油田(Bitsoil)是数据经济的新原油");
i18n.addTranslation('zh-CN', "Installation", "FixePunchline", "B", "快在世上最新资源中索取您应得的份额");

i18n.addTranslation('zh-CN', "Installation", "nextToVideo", "title", "观看视频");
i18n.addTranslation('zh-CN', "Installation", "nextToVideo", "content", "探索机器人如何收取数据油田(Bitsoil)。");

i18n.addTranslation('zh-CN', "WhoWeAre", "A", "<a href=\"http://larbitslab.be/\" target=\"_blank\">LarbitsSisters</a> 是一个来自比利时布鲁塞尔的艺术家二人组，跨界艺术、科技、政治和社会问题领域。他们的工作起源于对新媒体的共同迷恋，将研究和艺术实践融合到项目中，探索可追溯性、数据处理、网络分析、算法，自动化和信息交互的可能性。核心是探讨公共和私人，线上和线下，肆无忌惮的技术发展信仰和日常生活之间的摩擦。新媒体的策略和实行是通过上述调查及创造工具，衍生出对当今社会的关键问题提出质疑的作品。");
i18n.addTranslation('zh-CN', "WhoWeAre", "B", "2011年，LarbitsSisters成立了Larbitslab数字可视化研究实验室。Larbitslab汇集了艺术家和科学家，共同围绕网络问社会中产生的问题进行探讨。Larbitslab的特别之处在于其方法论，它结合了观察实践、媒体分析以及技术和社会影响的研究。");
i18n.addTranslation('zh-CN', "WhoWeAre", "C", "数据油田税运动源于创作人员对数据经济的增长、财富、社会公平、税收滥用以及解决互联网巨头逃税行为的共同观点。数据油田税运动可看作是黑客行为。对于这个项目，我们主要与程序员合作，感谢Jenny Mainframe, <a href=\"http://ogre.be/\" target=\"_blank\">Vincent Evrard</a>, <a href=\"http://crucifixarnaud.com/\" target=\"_blank\">Arnaud Crucifix</a>, Fleur Wirtz 和 Peng! collective, 更要特别感谢 <a href=\"https://imal.org/\" target=\"_blank\">iMAL</a> 自项目开始以来的不断支持。在武汉的展览中，特别感谢：方维，Big House当代艺术中心的创始人；黄晶晶，项目经理；郑富贵，项目助理和中国地质大学（武汉）的实习生。");
i18n.addTranslation('zh-CN', "WhoWeAre", "D", "本次活动的核心是为互联网资源重新分配提出新愿景。数据油田税(bitsoil tax)是人们创造的数据经济新能源或新油田，“数据油田”(bitsoil)针对数据征收微税。“数据油田”(bitsoil)为了人民、对99%征税，为了社会普通民众征税，抵制不平等现象，恢复数据经济中的公正平等。您可通过生成个人收税机器人参与到我们的活动中来。");

i18n.addTranslation('zh-CN', "Bitsoil", "info", "A", "这是数据油田(bitsoil)快闪流量税和黑客运动期间，机器人根据用户生成数据而申请的总税款，以0.0427%的税率计算。以每年两千亿条推文估算，可收取税款85亿美元。");
i18n.addTranslation('zh-CN', "Bitsoil", "info", "B", "数据油田税（Bitsoil Tax）旨在动员使用社交媒体平台或其它科技工具的用户对自己贡献的流量数据征收微税，从而呼吁数字经济财富的公平再分配。");

i18n.addTranslation('zh-CN', "FAQ", "A", "hash", "Who");
i18n.addTranslation('zh-CN', "FAQ", "A", "title", "这项运动是针对哪一类人群设计的？");
i18n.addTranslation('zh-CN', "FAQ", "A", "content", "这项运动向所有人开放。针对生成数据匿名用户，匿名大量生产个人数据、99%的人群、普通民众、难民、那些申请庇护者的人、移民以及非政府组织。此次运动具有双重目的，一方面，鉴于当前数据资源全部集中在互联网巨头手中，运动旨在唤起人们对新资源分配不均衡的认识，以及呼吁反对不平等及对资源的重新分配；另一方面，它为数据社会中的财富再分配提供了新视角。这场运动致力于唤醒沉睡在社会大众、参与者、非政府组织者、创始人、组织者和理论家中的积极力量。");
i18n.addTranslation('zh-CN', "FAQ", "B", "hash", "twitter");
i18n.addTranslation('zh-CN', "FAQ", "B", "title", "用户机器人在Twitter上都做些什么？");
i18n.addTranslation('zh-CN', "FAQ", "B", "content", "Twitter机器人可执行多种任务。分为勘探机器人，宣传机器人和税收机器人。<br/><br/>勘探机器人经过训练后可在Twitter上检测用户账号，寻找与活动主题相关的关键词或操作。IBM的Watson自然语言分类器已被用于训练机器人发现预设的关键词。<br/><br/>宣传机器人用于在已发现的用户账号中触发弹出式推文，包含此次活动的宣传视频，接着引导用户进入运动宣传网站。用户在网站上收到邀请，创建个人机器人或机器人小组，向科技公司申请税收。<br/><br/>所有机器人都配备了预编程的税务计算机制，并计算与活动相关的在线行为所对应的（虚构的）税额。通过这种机制，机器人能够在运动参与者之间重新分配这一附加值。");
i18n.addTranslation('zh-CN', "FAQ", "C", "hash", "bot");
i18n.addTranslation('zh-CN', "FAQ", "C", "title", "我创建的机器人做什么？");
i18n.addTranslation('zh-CN', "FAQ", "C", "content", "当您创建一个机器人时，它将在指定时间内或根据指定频率执行您的要求。您可以通过个人客户经理追踪机器人行为。该界面允许您追踪机器人执行操作的行为，以及在此过程中生成的bitsoil数据价值。<br/><br/>如果我给机器人指派任务，比如向朋友发送数字明信片，会发生什么？<br/><br/>当您指派机器人向朋友发送明信片，您的朋友将会收到一张匿名明信片，来自一位关心您的数据价值并致力于争取更公平经济的挚友。<br/><br/>机器人发送朋友的数字明信片将无法追溯到您。通过利用TOR路由器网络，您的隐私将受到保护，同时也强调了匿名群体的存在合理性。这项运动针对大量互联网匿名用户，也由匿名用户发起。");
i18n.addTranslation('zh-CN', "FAQ", "D", "hash", "secure");
i18n.addTranslation('zh-CN', "FAQ", "D", "title", "它安全吗？");
i18n.addTranslation('zh-CN', "FAQ", "D", "content", "您配置发送推文和/或发送数字明信片的机器人无法追溯至您的Twitter账户甚至个人数据。此外，机器人利用TOR路由器网络生成推文，这意味着我们的服务器和网站IP地址是隐藏的。<br/><br/>当我用分享按钮发布推文时会发生什么？<br/><br/>当您在帐户上发布推文时，我们诚挚感谢您的支持以及传递信息，因为此举的寓意远不止是代表您对这项运动的正式支持。");
i18n.addTranslation('zh-CN', "FAQ", "E", "hash", "tax");
i18n.addTranslation('zh-CN', "FAQ", "E", "title", "机器人真的会收税吗？");
i18n.addTranslation('zh-CN', "FAQ", "E", "content", "税收机器人只限于向互联网巨头提出税收申请。此外，机器人还提醒国家领导人，并强调指出拟议税收制度所能带来的利润。");
i18n.addTranslation('zh-CN', "FAQ", "F", "hash", "calculated");
i18n.addTranslation('zh-CN', "FAQ", "F", "title", "如何计算税额？");
i18n.addTranslation('zh-CN', "FAQ", "F", "content", "机器人执行的每个操作都会生成数据和数据油田（bitsoil），是转换为数据共和国（bitRepublic）的虚拟货币数据油田（bitsoil）的主要资源。数据油田（bitsoil）的总额由在线动作所涉及的数据位数和字节数转换得出。<br/><br/>粗略地说，一条140个字符的推文等于200个字节，等于1微数据油田（bitsoil）。同样地，当您创建机器人后，您在运动网站上操作的所有微动作都会根据流程中生成的位和字节转换为数据油田(bitsoil)。<br/><br/>那税款呢？ 我是否真的将收到机器人收集并在参与者之间重新分配的税款？<br/><br/>不是。机器人实际上只能申请缴税，以及提高人们对拟议税收制度所能带来的益处的认知");
i18n.addTranslation('zh-CN', "FAQ", "G", "hash", "leader");
i18n.addTranslation('zh-CN', "FAQ", "G", "title", "如果找不到我想要发送推文的那位国家首领怎么办？");
i18n.addTranslation('zh-CN', "FAQ", "G", "content", "请发送电子邮件至：<a href=\"{$address}\" >campaign@bitsoil.tax</a>");